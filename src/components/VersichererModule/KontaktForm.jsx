...
const KontaktForm = ({ versicherer, kontakt, onSaveSuccess, onCancel }) => {
  ...
  const handleArtChange = (option) => {
    ...
    const newArt = currentArt.includes(option) ? currentArt.filter(a => a !== option) : [...currentArt, option];
    setFormData(prev => ({ ...prev, art: newArt.join(',') }));
  };
  ...
  return (
    <form className="kontakt-form" onSubmit={handleSubmit}>
      ...
      <div className="form-row">
        ...
        <div className="form-group">
          <label>Telefon *</label>
          <input type="tel" name="telefon" value={formData.telefon} onChange={handleInputChange} required />
        </div>
      </div>
      <div className="form-group">
        ...
        <div className="checkbox-group">
          {artOptions.map(option => (
            <label key={option} className="checkbox-label">
              <input type="checkbox" checked={formData.art.includes(option)} onChange={() => handleArtChange(option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <div className="form-group">
        <label>
          <input type="checkbox" name="direkt_erreichbar" checked={formData.direkt_erreichbar} onChange={handleInputChange} />
      ...
      <div className="form-group">
        <label>Notizen</label>
        <textarea name="notizen" value={formData.notizen} onChange={handleInputChange} rows="3" />
      </div>
      <div className="form-actions">
        <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Speichert...' : 'Speichern'}</button>
